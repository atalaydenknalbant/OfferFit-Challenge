...
class DataCleaner:
    ...
    def adjust_dtype(self, types: Dict[str, Any]) -> None:
        try:
            ...
            self.history.append(("Adjusted dtypes using {}".format(types), self.current.copy(deep=True)))
        except Exception as ex:
            logging.error(ex)

    def impute_missing(self, columns: List[str]) -> None:
        try:
            self.current = self.current.fillna(self.current.mean()[columns])
            self.history.append(("Imputed missing in {}".format(columns), self.current))
        except Exception as ex:
            logging.error(ex)
